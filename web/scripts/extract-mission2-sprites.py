...
def resolve_map_paths(mission_id: str) -> tuple[Path, Path, Path, Path, Path, Path]:
    map_dir = ROOT / "public" / "maps" / mission_id
    return (
        map_dir,
        map_dir / f"{mission_id}-map.png",
        map_dir / f"{mission_id}-map.meta.json",
        map_dir / f"{mission_id}-composite.json",
        map_dir / "sprites",
        map_dir / "sprite-catalog.json",
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--mission",
        default="mission2",
        help="Mission folder id under public/maps/ (e.g. mission2, mission3).",
    )
    args = parser.parse_args()

    _map_dir, png_path, meta_path, composite_path, sprites_dir, catalog_path = resolve_map_paths(
        args.mission
    )

    if not png_path.exists() or not meta_path.exists() or not composite_path.exists():
        raise SystemExit(
            f"missing inputs — run extract-mission2-composite.py --mission {args.mission} first"
        )
    ...
    catalog_path.write_text(json.dumps(catalog_obj, indent=2), encoding="utf-8")
    ...
    print(f"[sprites] wrote sprite catalog -> {catalog_path}")
