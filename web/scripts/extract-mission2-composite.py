...
def resolve_map_paths(mission_id: str) -> tuple[Path, Path, Path, Path, Path]:
...
def main() -> None:
    ...
    _map_dir, png_path, meta_path, out_path, debug_path = resolve_map_paths(args.mission)
    ...
    if args.dump_labels:
        dump_labels(label_map, white_mask, debug_path)
...
def dump_labels(label_map: np.ndarray, white_mask: np.ndarray, debug_path: Path) -> None:
    palette = {
        L_VOID: (0, 0, 0),
        L_GRASS: (60, 160, 80),
        L_WATER: (60, 120, 200),
